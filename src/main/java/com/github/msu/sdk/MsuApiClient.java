...
import com.github.msu.sdk.authentication.credentials.providers.ClasspathPropertyFileCredentialsProvider;
...
public class MsuApiClient {
	...
	private static final Set<String> SHOULD_MASK_PARAMS = new HashSet<>(
			Arrays.asList(Param.CARDPAN.name(), Param.CARDCVV.name(), Param.CARD_EXPIRY.name(), Param.CARDEXPIRY.name(),
					Param.MERCHANTPASSWORD.name(), Param.MERCHANTUSERPASSWORD.name()));
	...
	public void log(ApiRequest request, ApiResponse response, long totalTimeMs) {
		StringBuilder sb = new StringBuilder("- Total Request Time: ").append(totalTimeMs).append(" ms")
				.append(System.lineSeparator()).append("- Authentication: ").append(request.getAuthentication())
				.append(System.lineSeparator()).append("- Request Body:").append(System.lineSeparator());
